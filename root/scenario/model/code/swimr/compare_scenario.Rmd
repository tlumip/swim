...
colnames(mpo_controls)[1] <- "MPOmodeledzones" # To match the field in AllZones in the database
```
...
```{r se}
se <- left_join(
  extract_se(db1, "MPOmodeledzones") %>% rename(reference = y),
  extract_se(db2, "MPOmodeledzones") %>% rename(current   = y)
) %>%
...
compare_sevar(db1, db2, color_var = facet, color_levels = facet_levels, index_year=index_year)
```

```{r compare_se_mpo, fig.cap="Comparison of population and employment - all MPOs."}
compare_sevar(db1, db2, color_var = "MPO", index_year=index_year)
```

```{r compare_se_mpomodeled, fig.cap="Comparison of population and employment - all MPOs Modeled.", fig.width=8, fig.height=12}
compare_sevar(db1, db2, color_var = "MPOmodeledzones", index_year=index_year)
```
...
compare_vmt(db1, db2, facet, facet_levels, index_year=index_year)
```

```{r compare_vmt_mpo, fig.cap="Comparison of VMT by facility type - all MPO's."}
compare_vmt(db1, db2, "MPO", index_year=index_year)
```
...
compare_trips(db1, db2, facet_var = facet, facet_levels = facet_levels, index_year=index_year)
```


```{r compare_mode_mpo, fig.cap="Comparison of trip productions by mode - all MPOs."}
compare_trips(db1, db2, facet_var = "MPO", index_year=index_year)
```
...
compare_tlfd(db1, db2, facet, facet_levels, cumulative = TRUE, years = years, index_year=index_year)
```

```{r compare_tlfd_mpo, fig.cap = "Comparison of trip length frequency distribution - all MPO's."}
compare_tlfd(db1, db2, "MPO", years = years, cumulative = TRUE, MPOs, index_year=index_year)
```


## Log Sums
```{r compare_logsum, fig.cap = "Mode choice model logsums."}
compare_logsums(db1, db2, facet, facet_levels, index_year=index_year)
```

```{r compare_logsum_mpo, fig.cap = "Mode choice model logsums - all MPO's"}
compare_logsums(db1, db2, "MPO", MPOs, index_year=index_year)
```
...
compare_wapr(db1, db2, facet, facet_levels, index_year=index_year)
```

```{r compare_wapr_mpo, fig.cap="Comparison of workforce participation by sector - all MPO's."}
compare_wapr(db1, db2, "MPO", index_year=index_year)
```
